(function (angular) {
    ...
    /**
     * @param $rootScope - Not proud of this dependency, but will hopefully remove some hard-hitting watchers
     * @param $window
     * @param _, lodash library, really need to get around to implementing ES6 for imports of this stuff
     * @param utils
     * @constructor
     */
    function Service ($rootScope, $window, _, utils) {
        var self = this;
        ...
        function setCurrentEntity (o) {
            if ( !_.isEqual(_currentEntity, o) ) {
                _currentEntity = o;
                $rootScope.$broadcast('cg.current-entity.update', _currentEntity);
            }
        }
        ...
        function mobileCheck () {
            ...
            var agentCheck = $window.navigator.userAgent || $window.navigator.vendor || $window.opera;

            return regex.test(agentCheck) || regex2.test(agentCheck.substr(0, 4));
        }

        function getEntityList () { return entityList; }
        ...
        // WARNING, MUTABLE!  Everyone will see if you update properties of this
        function getConnectionObj () { return connectionObj; }
        ...
        function setCurrentView (s) {
            if ( utils.isString(s) && availableViews.includes(s) ) {
                currentView = s;
            }
        }
