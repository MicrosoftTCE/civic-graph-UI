(function(angular, d3) {
    ...
    function Directive(_, cgService) {
        ...
        function linkFunction($scope, element) {
            ...
            function loadData(data) {
                if (cgService.mobileCheck()) {
                    drawNetworkMobile(data);
                }
                else {
            ...
            function drawNetworkGraph(data) {
                ...
                var entityArray = data.entityArray;
                var connectionArray = data.connectionArray;
            ...
            function drawNetworkMobile(entityArray) {
                var data = {
                ...
                data.nodes
                    .filter(function filterEntity(e) {
                        return isDef(entityArray.find(function(v) { return v.id === e.id; }));
                    })
                    .forEach(
                        function (d) {
                ...
                function tick() {
                    function drawEntityName(d) {
                        ...
                        context.strokeText(
                            name,
                            Math.max(4.5 * k, Math.min(width - 4.5 * k, d.x + offsets[d.type][0])) - name.length * 2,
                            Math.max(4.5 * k, Math.min(height - 4.5 * k, d.y + offsets[d.type][1])) + 10,
                            100
                        );
                    }
                    ...
                    _.forEach($scope.connections, function (connections, type) {
                        connections.forEach(function (d) {
                            ...
                            if (!(isConTypeDefined && isEntityTargetTypeDefined && isEntitySourceTypeDefined)) {
                                return;
                            }

                            if (isDef($scope.currentLocation)) {
                                if (d.source.name in $scope.currentLocation.dict
                                    && d.target.name in $scope.currentLocation.dict) {
                                    context.beginPath();
                                    ...
                                    context.moveTo(
                                        Math.max(
                                            4.5 * k,
                                            Math.min(width - 4.5 * k, d.source.x + offsets[d.source.type][0])
                                        ),
                                        Math.max(
                                            4.5 * k,
                                            Math.min(height - 4.5 * k, d.source.y + offsets[d.source.type][1])
                                        )
                                    );
                                    context.lineTo(
                                        Math.max(
                                            4.5 * k,
                                            Math.min(width - 4.5 * k, d.target.x + offsets[d.target.type][0])
                                        ),
                                        Math.max(
                                            4.5 * k,
                                            Math.min(height - 4.5 * k, d.target.y + offsets[d.target.type][1])
                                        )
                                    );
                            ...
                            else if (!isDef($scope.currentEntity)
                    ...
                    entityNames.forEach(drawEntityName);
                }
                ...
                $scope.$on('toggleNode', tick);
                $scope.$on('toggleLink', tick);
                $scope.$on('changeSizeBy', tick);
                $scope.$on('selectItem', function (event, item) {
                    if (item.type === 'location') {
            ...
            $scope.$watch("data", loadData, true);
        }
    ...
    angular.module('mscg.network.graph')
        .directive("mscgNetworkGraph", Directive);
